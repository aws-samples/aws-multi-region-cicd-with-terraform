...
usage_help="Usage: $0 [-t prod_<region>/tooling/<version>] [-b <tf_backend_config_prefix>] [-g <global_resource_deploy_from_region>] [-r <tf_state_region>]"
...
if [ -z "$tag" ]; then tag="prod_$tf_state_region/tooling/1.0"; fi
...
REGION=$(if [[ ${TARGET_DEPLOYMENT_SCOPE} == *"global"* ]];then echo "${GLOBAL_RESOURCE_DEPLOY_FROM_REGION}"; else echo "${TARGET_DEPLOYMENT_SCOPE}"; fi)
